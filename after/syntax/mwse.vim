" Vim syntax file
" Language: MWSE Script (based on OBSEScript by Ulthar Seramis)
" Maintainer: mg979.git@gmail.com
" Latest Revision: sab 04 mag 2019

if exists("b:current_syntax")
	finish
endif

let b:current_syntax = 'mwse'

syn match mwseInt '[-+]\?\<\d\+\>' display
syn match mwseFloat '[-+]\?\<\d\+\>\.\<\d\+\>' display
syn match mwseNumOp '\V - \| / \| + \| * \| == \| <= \| >= \| < \| > \| != ' display
syn match mwseBracket '\V\( \)\?( \|) \| )' display

syn match mwseActor '[[:alnum:]]\+\->' display contains=mwseArrow
syn match mwsePlayer '\<[Pp]layer\>\(\->\)\?' display contains=mwseArrow
syn match mwseArrow '\->' display contained

syn region mwseComment start=";" end="$" keepend contains=mwseToDo
syn region mwseString start=/"/ end=/"/

syn keyword mwseToDo contained TODO todo Todo ToDo FIXME fixme NOTE note
syn keyword mwseTypes short long float ref reference array_var string_var int
syn keyword mwseCondition if else elseif endif If Else Elseif Endif ElseIf EndIf Label label GoTo Goto goto while While endwhile EndWhile Endwhile
syn keyword mwseX ifx Ifx IfX setx SetX Setx
syn keyword mwseOperator set to
syn keyword mwseScript StartScript ScriptRunning StopScript startscript scriptrunning stopscript
syn keyword mwseBlock Begin End begin end return Return

syn keyword mwseFunction
      \ AIActivate
      \ AIEscort
      \ AIEscortCell
      \ AIFollow
...
if !exists("did_mwse_inits")

	let did_mwse_inits = 1
	hi def link mwseComment Comment
	hi def link mwseString String
	hi def link mwseInt Number
	hi def link mwseFloat Float
	hi def link mwseNumOp Operator
	hi def link mwseBracket Special
	hi def link mwseToDo Todo
	hi def link mwseTypes Type
	hi def link mwseCondition Conditional
	hi def link mwseOperator Operator
	hi def link mwsePlayer Keyword
	hi def link mwseBlock WarningMsg
  hi def link mwseActor Statement
  hi def link mwseArrow Operator
  hi def link mwseX WarningMsg
  hi def link mwseScript WarningMsg
	hi def link mwseGetFunc Number
	hi def link mwseSetFunc Operator
	hi def link mwseModFunc Type
	hi def link mwseEffects Special
	hi def link mwseFunction Function
	hi def link mwseFunctionX WarningMsg
endif
